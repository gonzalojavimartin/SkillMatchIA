# Entrenamos modelo NER para identificar tecnologías específicas en descripciones de trabajo
import spacy
...
# Agregamos un label especifico para estos terminos
ner.add_label("TECH")

# Inicializamos datos de entrenamiento
train_data = [
...
with nlp.disable_pipes(*other_pipes):
    ...
    # Entrenamos el modelo NER
    optimizer = nlp.begin_training()

    # 20 iteraciones mezclando aleatoriamente train_data en cada iteración
    for itn in range(20):
        print(f"Starting iteration {itn}")
        random.shuffle(train_data)
        losses = {}

        # El modelo se actualiza utilizando los ejemplos, ajustando los pesos para mejorar la detección de entidades "TECH".
        for batch in spacy.util.minibatch(train_data, size=2):
            examples = [Example.from_dict(nlp.make_doc(text), annotations) for text, annotations in batch]
            # El parámetro drop=0.5 introduce algo de ruido para evitar el sobreajuste durante el entrenamiento.
